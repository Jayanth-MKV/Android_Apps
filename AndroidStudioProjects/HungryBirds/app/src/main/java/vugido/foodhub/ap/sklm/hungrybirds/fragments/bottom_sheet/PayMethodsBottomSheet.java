...
public class PayMethodsBottomSheet extends BottomSheetDialogFragment implements View.OnClickListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        PPay=view.findViewById(R.id.ppay);
...
//        getDialog().setCancelable(false);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.cash_on_delivery:
                // action..
                pay_method_interface.payMethodInterface(CASH_ON_DELIVERY);
                Objects.requireNonNull(getDialog()).dismiss();
                break;
            case R.id.ppay:
                Toast.makeText(context,"Will be  activated soon",Toast.LENGTH_LONG).show();
                break;
            case R.id.gpay:
                Toast.makeText(context,"Google Pay is Currently not activated",Toast.LENGTH_LONG).show();
                break;
    ...
    public interface  PAY_METHOD_INTERFACE{
        ...
        void payMethodInterface(int pm);
    }
}
