...
public class SquareMediumRecyclerViewAdapter extends RecyclerView.Adapter<SquareMediumRecyclerViewAdapter.MyViewHolder>implements Filterable {
    ...
    List<SECItem> categoriesItemList;
    List<PRODUCTSMENUItem> squareMediumModelList;
    Context context;
     List<PRODUCTSMENUItem> filteredList;
     AddToCart addToCart;

    public SquareMediumRecyclerViewAdapter(List<PRODUCTSMENUItem> squareMediumModelList, Context context,List<SECItem> categoriesItemList) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        PRODUCTSMENUItem squareMediumModel=filteredList.get(position);
    ...
    private Filter filter=new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            List<PRODUCTSMENUItem> filteredList = new ArrayList<>();
            if (constraint == null || constraint.length() == 0) {
                filteredList.addAll(squareMediumModelList);
            } else {
                String filterPattern = constraint.toString().toLowerCase().trim();
                for (PRODUCTSMENUItem item : squareMediumModelList) {
                    if ( item.getENAME().toLowerCase().contains(filterPattern) || getCat(item.getSID()).contains(filterPattern) || item.getTAG().contains(filterPattern)) {
        ...
        protected void publishResults(CharSequence constraint, FilterResults results) {
            ...
            filteredList.clear();
            filteredList.addAll((Collection<? extends PRODUCTSMENUItem>) results.values);
...
//            value=itemView.findViewById(R.id.textView2);
    ...
    private  String getCat(int id){
        for (SECItem categoriesItem:categoriesItemList){
